...
Date.daysdiff=function(first,second,offset){
	var oneDay = 24*60*60*1000;
	var diff=Math.round(Math.abs((second.getTime() - first.getTime())/(oneDay)));
	return offset?diff:diff+1;
}
Date.prototype.moveToFirstDayOfQuarter=function() {
	var y=this.getFullYear();
	var q=3*Math.floor(this.getMonth() / 3);
	this.set({day:1,month:q,year:y});
}
...
Date.getDaysInQuarter=function(y,quarter){
	if(quarter==0){
		return Date.isLeapYear(y)?91:90;
	}
	var dinq=[90,91,92,92];
	return dinq[quarter];
}
